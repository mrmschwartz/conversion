...
Converter.prototype.addUnit = function (options) {
  if (!options) {
      throw new Error('No configuration passed to "addUnit"')
      return
  }

  var system = options.system
  var measure = options.measure
  var abbr = options.abbr
  var singular = options.singular
  var plural = options.plural
  var toAnchor = options.toAnchor

  if (system !== 'imperial' && system !== 'metric') {
    throw new Error('Invalid "system" configuration. Specify "metric" or "imperial"')
    return
  }

  var possibleMeasures = ['length', 'mass', 'volume', 'each', 'temperature', 'time', 'digital']
  if (possibleMeasures.indexOf(measure) < 0) {
    throw new Error('Invalid "measures" configuration. Specify: "' + possibleMeasures.join(", ") + "")
    return
  }

  if (isNaN(toAnchor)) {
    throw new Error('"toAnchor" must be a valid number.')
    return
  }

  if (!abbr || !singular || !plural) {
    throw new Error('"abbr", "plural", and "singular" are required configuration options.')
    return
  }

  measures[measure][system][abbr] = {
    name: {
      plural: plural,
      singular: singular,
    },
